...
export class CategoryRepository extends BaseRepository<Category> {
  findByFilters(filters: ICatalogFilters, requestedPaths: INormalizedGqlRequestedPaths): Promise<Category[]> {
    ...
    if (filters?.category) {
      queryBuilder.andWhere(`${requestedPaths.root}.parent = :parent`, {parent: filters.category});
    } else {
