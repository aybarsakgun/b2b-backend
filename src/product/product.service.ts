...
export class ProductService {
  ...
  async findById(requestedPaths: INormalizedGqlRequestedPaths, id: string): Promise<Product> {
    if (!id) {
      return null;
    }
    return await this.productRepository.getPopulatedQuery(requestedPaths)
      .where(`${requestedPaths.root}.id = :id`, { id })
      .getOne();
  }

  async findAll(requestedPaths: INormalizedGqlRequestedPaths, paginationInput: IPaginationInput): Promise<IPaginationResult<Product>> {
