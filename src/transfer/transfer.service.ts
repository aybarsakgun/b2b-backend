...
export class TransferService {
  ...
  async importUsers(users: UserDto[]) {
    ...
    try {
      ...
      const willBeDeleteUsers = users.filter(user => !users.some(userDto => userDto.customerId === user.customerId)).map(user => user.customerId);
  ...
  async importProducts(products: ProductDto[]) {
    ...
    const fillCategoryValues = async (category: CategoryDto, productId: number, parentId: number = null) => {
      categoryValues.push({
        id: category.id,
        name: category.name,
    ...
    const fillProductValues = async (product: ProductDto, parentId: number = null) => {
      let brand = null;
      if (product.brand) {
        const findBrand = await queryRunner.manager.query(`SELECT id FROM brand WHERE brand.name = ?`, [product.brand]);
        brand = findBrand.length ? findBrand[0].id : null;
      }
      console.log(brand);
      let model = null;
      if (brand && product.model) {
        const findModel = await queryRunner.manager.query(`SELECT model.id FROM brand, model WHERE model.brand_id = brand.id AND model.name = ? AND brand.id = ?`, [product.model, brand]);
        model = findModel.length ? findModel[0].id : null;
      }
      console.log(model);
      if (product.brand && !brand) {
        const brandInsert = await queryRunner.manager.insert(Brand, new Brand({
          name: product.brand,
      ...
      if (product.model && !model && brand) {
        const modelInsert = await queryRunner.manager.insert(Model, new Model({
          name: product.model,
          brand: brand
        }));
    ...
    const executeProductToCategoryValues = async () => {
      for (const productToCategory of productToCategoryValues) {
        await queryRunner.manager.query(`REPLACE INTO product_to_category (product_id, category_id) VALUES (?, ?)`, [productToCategory.productId, productToCategory.categoryId]);
      }
    };
    ...
    const executeProductUnits = async (products: ProductDto[]) => {
      for (const product of products) {
        for (const productUnit of product.units) {
          const executeProductUnit = await queryRunner.manager.query(`INSERT INTO product_unit (
          value,
          barcode,
          default_price_order,
          list_price_order,
          ...
          if (!executedProductUnitId || executedProductUnitId === 0) {
            const findProductUnit = await queryRunner.manager.query(`SELECT id FROM product_unit WHERE product_id = ? AND value = ?`, [product.id, productUnit.value]);
            executedProductUnitId = findProductUnit.length ? findProductUnit[0].id : null;
          }
          if (executedProductUnitId) {
            for (const productPrice of productUnit.prices) {
              await queryRunner.manager.query(`INSERT INTO product_price (
          value,
          currency,
          price_order,
          unit_id) VALUES (
    ...
    const executeWarehouses = async (products: ProductDto[]) => {
      for (const product of products) {
        for (const warehouse of product.warehouses) {
          await queryRunner.manager.query(`INSERT INTO warehouse (
        product_id,
        quantity,
        warehouse_id,
        date,
