...
export class NormalizeGqlResolveInfo {
  ...
  private constructor({fragments, fieldNodes}: GraphQLResolveInfo, isPaginated?: boolean) {
  ...
  private mapJoins = (selections: readonly SelectionNode[]): INormalizedGqlRequestedJoin[] => {
    return selections.reduce<INormalizedGqlRequestedJoin[]>((paths, field, i) => {
  ...
  private requestedPaths(): INormalizedGqlRequestedPaths {
    ...
    const fieldPaths: INormalizedGqlRequestedPaths = {root: fieldJoins.parent, relations: []};
  ...
  static readonly RequestedPaths = createParamDecorator<{
    isPaginated: boolean
  }, ExecutionContext>((options, ctx) =>
    new NormalizeGqlResolveInfo(GqlExecutionContext.create(ctx).getInfo<GraphQLResolveInfo>(), options?.isPaginated).requestedPaths()
  )
}
